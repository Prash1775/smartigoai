...
class GreOnboardingScreen extends ConsumerStatefulWidget {
  final String uid;

  const GreOnboardingScreen({super.key, required this.uid});

  @override
  ConsumerState<GreOnboardingScreen> createState() =>
      _GreOnboardingScreenState();
}

class _GreOnboardingScreenState extends ConsumerState<GreOnboardingScreen> {
  int _currentStep = 0;
  bool isLoading = false;

  // Step 1: Target Score
  int selectedScore = 320;
  ...
  Future<void> _handleComplete() async {
    ...
    try {
      await FirestoreService.updateUser(widget.uid, {
        'examType': ExamType.gre.displayName,
  ...
  Widget _buildTargetScoreStep() {
    ...
    return Column(
      ...
      children: [
        ...
        Text('GRE total score ranges from 260 to 340.',
        ...
        Slider(
          value: selectedScore.toDouble(), min: 260, max: 340, divisions: 80,
          label: '$selectedScore',
        ...
        Row(
          ...
          children: [
            _quickScore('280', 280), _quickScore('300', 300),
            _quickScore('320', 320), _quickScore('340', 340),
          ],
        ),
      ],
    );
  ...
  Widget _buildCurrentLevelStep() {
    final theme = Theme.of(context);
    final levels = [
      {'value': 'beginner', 'title': 'Beginner', 'subtitle': 'First time taking GRE', 'icon': Icons.school_outlined},
      {'value': 'intermediate', 'title': 'Intermediate', 'subtitle': 'Some prep done, need more practice', 'icon': Icons.trending_up},
      {'value': 'advanced', 'title': 'Advanced', 'subtitle': 'Retaking for higher score', 'icon': Icons.star_outlined},
    ];
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
  ...
  Widget _buildExamDateStep() {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('When is your\nGRE exam?', style: theme.textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold)),
        const SizedBox(height: 8),
